let answer = Math.floor(Math.random() * 7); // Gera um número aleatório de 0 a 6
...
submitBtn.addEventListener('click', function() {
  ...
  if(aposta <= score){// verifica se o valor apostado é menor que o valor restante
    if (guess === answer) {// Verifica se a resposta está correta
        if (aposta == score) {
          score += aposta * aposta;
        } else {
          score += aposta * 6; // Multiplica a aposta por 6
          resultText.textContent = 'Parabéns! Você acertou o número.'; // Exibe a mensagem de acerto
          resultText.style.color = '#4caf50'; // Muda a cor da mensagem de resultado para verde
        }
      } else {
        score -= aposta; // Remove 50 pontos da pontuação
        resultText.textContent = 'Que pena! Você errou o número.'; // Exibe a mensagem de erro
